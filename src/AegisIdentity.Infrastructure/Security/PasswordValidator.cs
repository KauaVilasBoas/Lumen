...
/// HaveIBeenPwned database via <see cref="IPwnedPasswordsClient"/>.
/// <para>
/// Structural rules are evaluated first; the HIBP call is only made when all
/// structural checks pass to avoid burning an external HTTP request on a
/// clearly weak password.
/// </para>
/// </summary>
public sealed class PasswordValidator : AbstractValidator<PasswordValidationContext>, IPasswordValidator
{
    ...
    private static bool IsSpecialCharacter(char c)
        => ValidationLimits.PasswordSpecialCharacters.Contains(c);
