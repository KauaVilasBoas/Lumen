...
internal sealed class ProfileRepository : IProfileRepository
{
    ...
    public ProfileRepository(LumenAuthorizationDbContext dbContext)
    {
        _dbContext = dbContext;
    }

    public Task<Profile?> FindByIdAsync(Guid id, CancellationToken ct = default)
        => _dbContext.Profiles.FirstOrDefaultAsync(p => p.Id == id, ct);

    public Task<Profile?> FindByNameAsync(string name, CancellationToken ct = default)
        => _dbContext.Profiles.FirstOrDefaultAsync(p => p.Name == name, ct);
    ...
    public async Task<IReadOnlyList<Profile>> GetProfilesByUserIdAsync(Guid userId, CancellationToken ct = default)
        => await _dbContext.UserProfiles
                           .Where(up => up.UserId == userId)
                           .Join(_dbContext.Profiles, up => up.ProfileId, p => p.Id, (up, p) => p)
                           .AsNoTracking()
                           .ToListAsync(ct);

    public async Task<IReadOnlyDictionary<Guid, IReadOnlyList<Profile>>> GetProfilesByUserIdsAsync(
        IReadOnlyList<Guid> userIds,
        CancellationToken ct = default)
    {
        var rows = await _dbContext.UserProfiles
            .Where(up => userIds.Contains(up.UserId))
            .Join(_dbContext.Profiles, up => up.ProfileId, p => p.Id, (up, p) => new { up.UserId, Profile = p })
            .AsNoTracking()
            .ToListAsync(ct);

        return rows
            .GroupBy(r => r.UserId)
            .ToDictionary(g => g.Key, g => (IReadOnlyList<Profile>)g.Select(r => r.Profile).ToList());
    }
        ...
        CancellationToken ct = default)
    {
        var pairs = await _dbContext.UserProfiles
            .Where(up => userIds.Contains(up.UserId))
            .Join(_dbContext.PermissionProfiles, up => up.ProfileId, pp => pp.ProfileId, (up, pp) => new { up.UserId, pp.PermissionId })
            .Join(_dbContext.Permissions, r => r.PermissionId, p => p.Id, (r, p) => new { r.UserId, r.PermissionId })
            .Distinct()
            .AsNoTracking()
            .ToListAsync(ct);

        return pairs.GroupBy(r => r.UserId).ToDictionary(g => g.Key, g => g.Count());
    }
    ...
    public async Task<IReadOnlyList<Profile>> GetByIdsAsync(IReadOnlyList<Guid> ids, CancellationToken ct = default)
        => await _dbContext.Profiles.Where(p => ids.Contains(p.Id)).ToListAsync(ct);
    ...
    public Task<HashSet<string>> GetPermissionCodesByUserIdAsync(Guid userId, CancellationToken ct = default)
        => GetPermissionCodesByUserIdAsync(userId, scopeId: null, ct);
    ...
    public async Task<IReadOnlyList<PermissionProfile>> GetActivePermissionProfilesByProfileIdAsync(
        Guid profileId,
        CancellationToken ct = default)
        => await _dbContext.PermissionProfiles.Where(pp => pp.ProfileId == profileId).ToListAsync(ct);
        ...
        CancellationToken ct = default)
    {
        ...
        return rows
            .GroupBy(pp => pp.ProfileId)
            .ToDictionary(g => g.Key, g => (IReadOnlyList<PermissionProfile>)g.ToList());
    }
    ...
    public async Task UpdatePermissionProfileAsync(PermissionProfile permissionProfile, CancellationToken ct = default)
    {
        _dbContext.PermissionProfiles.Update(permissionProfile);
        await _dbContext.SaveChangesAsync(ct);
    }

    public async Task<IReadOnlyList<Guid>> GetUserIdsByProfileIdAsync(Guid profileId, CancellationToken ct = default)
        => await _dbContext.UserProfiles
